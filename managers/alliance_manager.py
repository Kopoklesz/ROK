...
class AllianceManager:
    ...
    def run_help(self, task_data=None):
        ...
        FIX: Először fix koordinátákat próbál, ha nem sikerül, akkor template matching
        """
        log.separator('=', 60)
        ...
        # ===== STRATÉGIA 1: FIX KOORDINÁTÁK (gyorsabb, megbízhatóbb) =====
        if hand_locations:
            log.info(f"[Alliance] Fix koordináták próbálása ({len(hand_locations)} db)...")

            for idx, location in enumerate(hand_locations, 1):
                x, y = location.get('x', 0), location.get('y', 0)

                log.info(f"[Alliance] Location #{idx}: ({x}, {y})")

                delay = wait_random(self.human_wait_min, self.human_wait_max)
                log.wait(f"Várakozás {delay:.1f} mp")
                time.sleep(delay)

                log.click(f"[Alliance] Fix koordináta kattintás → ({x}, {y})")
                safe_click((x, y))
        ...
        # ===== STRATÉGIA 2: TEMPLATE MATCHING (fallback) =====
        if not hand_template.exists():
            log.warning(f"[Alliance] hand.png template nem található: {hand_template}")
            log.info("[Alliance] Fix koordináták sem állnak rendelkezésre")
            log.info("TODO: Setup Wizard-dal állítsd be a hand_locations-t vagy hand.png template-et")
            log.separator('=', 60)
            return

        log.info(f"[Alliance] hand.png template matching fallback: {self.max_retries}x retry, threshold={self.threshold}")

        # ===== 10x RETRY LOOP =====
        for attempt in range(self.max_retries):
            log.separator('-', 60)
            log.info(f"[Alliance] Template matching próbálkozás {attempt+1}/{self.max_retries}")
            log.separator('-', 60)

            log.search(f"[Alliance] hand.png keresés (teljes képernyő) - threshold {self.threshold}...")

            coords = ImageManager.find_image(str(hand_template), threshold=self.threshold)

            if coords:
                log.success(f"[Alliance] Hand megtalálva → {coords} ({attempt+1}. próba)")
                ...
                log.info(f"[Alliance] Template nem található ({attempt+1}/{self.max_retries})")
